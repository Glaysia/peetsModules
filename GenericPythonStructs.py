...
from PHXCheckType import checkIsInstance
from PHXCheckType import checkIsInstanceArray
...
def indexListToCSV(indexList):
   adict = {
       "[": "",
       "]": "",
   }
   index = multiple_replace(str(indexList), adict)
   ...
   return multiple_replace(str(strList), adict)

class PHXException(RuntimeError):
   """
...
def phxExceptionHandler(tb,ex):
   """
   Rethrows exceptions as a PHXException

   Parameters:
      - ex: The exception to wrap.
      - tb: traceback exception format
   """
   print("phxExceptionHandler",tb,ex,'\n',file=sys.stderr)
   if isinstance(ex,GeneralError):
      raise PHXException(ex.Message) from ex
   else:
      raise PHXException() from ex


def PHXClassDecorator(decorator):
   """
   Used to decorate every method in a class with the given decorator
   """
   def apply(cl):
      """
      Applies the decorator to each function in the class.
      """
      for k, f in cl.__dict__.items():
         if inspect.isfunction(f):
            setattr(cl, k, decorator(f))
      return cl
   return apply


def PHXTException_Decorator(func):
   """
   Decorator to attach try/except to catch TExceptions when a function
   is called.
   """
   def wrapper(*args, **kwargs):
      try:
         # Must be updated so that phxPython references correct value.
         global phxPython
         phxPython = PHXPythonGlobals.clientFaker
         ...
         phxExceptionHandler(tracebackFormatException,ex)
...
class PHXScriptWrapperObject:
   """
   This object is passed to Script Wrapper scripts and provides
   the "wrapper" global object which provides runtime services for
   the wrapper.
   """
...
class PHXRunShareContext:
   ...
   def run(self, cmd, ignoreErrors=False, autoChangeDir=True, shell='sh'):
      ...
      @type cmd: string
      @param ignoreErrors: By default this code will throw an exception if the spawned process
         returns a non-zero errorlevel. Occasionally a program will return an errorlevel even
         if it succeeds. This option allows you to igore the errorlevel.
      ...
      @param shell: On UNIX you can specify which shell to use to launch the program. By
         default "sh" is used.
      @type shell: string
      @rtype: void
      """
...
class PHXRowFieldFile:
   """
   Class implements the parsing capabilities of the RowField type files from the
   fileWrapper facility. Typical use would be to write a function which creates
   an instance of this class and implements the parsing using the transferVar()
   function. Then, a mode can be passed in which specifies whether the function
   should read the template file, generate an input file, or parse an output file.

   You will want to read the documentation on the fileWrapper utility for more
   specifics on how exactly the file parsing and options work.
   ...
   def __init__(self, wrapper, mode):
      ...
      @param mode: valid mode types are:
         - GENERATE - This mode makes the row field file prepare to generate the output file by
            reading the template file, then replacing the specified values passed to transferVar()
         - PARSE - This mode makes the row field file read in values from the fileToParse when transferVar()
            is called.
         - READ_TEMPLATE - This mode makes the row field file read in values from the template
            file when transferVar() is called.
      @type mode: int
      @rtype: void
      """
   ...
   def markAsBeginning(self, find, occurrence=1, offset=0, regex=False):
      """
      Searches for the specified string, starting from the end of the file and working towards the beginning,
      and makes the row where the string was found act as row 1 for all subsequent calls until another
      markAsBeginning or clearMarks call.
   ...
   def readDouble(self, row, field):
      """
      Reads a value out of the input file. If mode is PARSE, the input file is the fileToParse,
      otherwise it is the template file. Returns it as a double value.
   ...
   def readLong(self, row, field):
      """
      Reads a value out of the input file. If mode is PARSE, the input file is the fileToParse,
      otherwise it is the template file. Returns it as a long value.
      @param row: The row of the file to read from
      @type row: int
      @param field: The field of the file to read from.
      @type field: int
   ...
   def readValue(self, row, field):
      """
      Reads a value out of the input file. If mode is PARSE, the input file is the fileToParse,
      otherwise it is the template file.
      @param row: The row of the file to read from
      @type row: int
      @param field: The field of the file to read from.
      @type field: int
   ...
   def setDelimiters(self, delimiters):
      """
      Sets the delimiters which should be used for parsing lines into fields. Acceptable values:
   ...
   def transferArray(self, var, rowStart, rowEnd, fieldStart, fieldEnd, resizable=False, fformat='', numDimensions=1):
      """
      Transfers the data for a particular array. If mode is GENERATE data is read from
      the var and put into the file, otherwise data is read from the file into the variable.
   ...
   def transferKeyVar(self, var, key, occurrence=1, rowOffset=0, field=1, fformat=''):
      """
      Transfers the data for a particular variable. If mode is GENERATE data is read from the
      var and put into the file, otherwise data is read from the file into the variable.
...
class PHXSimpleType:
   ...
   This class will fire a property change event when hasChanged(true) is called. The
   property event will always have the property name be "value" and the old and new
   values of null. Use toString or getValue functions to get the actual new value from
   the variable.
   """
...
class PHXSimpleArray(PHXSimpleType):
   """
   This class enables components to create resizeable arrays where the number of dimensions is
   variable as well as the size of each dimension.

   Typically you don't use this class directly but one of its sub-classes that are created and
   passed to you automatically.
   """
   ...
   def toString(self):
      """
      Returns the string form of this array. Puts each element in quotes, even if it is a
      number and backslashes any quote or backslash characters (" and \).
      @rtype: string
      """
   ...
   def resize(self, newSize):
      """
      Resize the nD array to the new size specified. Preserve elements if the # of
      dimensions stays the same, else initialize the array with default values.
...
class PHXDouble(PHXSimpleType):
   ...
   If the variable has an upper or lower bound and an attempt is made to store an invalid value,
...
class PHXRawFile(PHXSimpleType):
   ...
   def getIsBinary(self):
      """
      Tells whether or not the the file is binary. Simply switches on whether
   ...
   def markAsRead(self):
      ...
      records it's name and timestamp for use with the hasChanged() function
      @rtype: void
      """
...
class PHXRawFileArray(PHXSimpleArray):
   """
   The Phoenix type for File Array. This class has the following properties:
      - name: String
   """
...
class PHXScriptObject(PHXSimpleType):
   """
   The Phoenix type for object variable. This class adds the following features to the PHXSimpleType:
